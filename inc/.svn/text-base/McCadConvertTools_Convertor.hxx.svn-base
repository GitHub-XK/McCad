...
McCadConvertTools_Convertor(const Handle(TopTools_HSequenceOfShape)& theHSeqOfShp);
...
//! Returns the converted model. <br>
  Handle_TopTools_HSequenceOfShape Convert(const Handle(TopTools_HSequenceOfShape)& theHSeqOfShp) ;

//! Return the converted Model as a HSequenceOfShape
  Handle_TopTools_HSequenceOfShape GetConvertedModel() ;

//! Return the file name
  TCollection_AsciiString GetFileName() const { return myFileName; }
...
//! Returns true, if the conversion has been successfully performed.
  Standard_Boolean IsConverted() const;
